...
// Затем можно прочитать значение атрибута с помощью свойства dataset:
const sayHiValue = element.dataset.sayHi;
// В результате получим значение "yes":
console.log(sayHiValue); // выведет "yes"
...
const element1 = document.querySelector("li:nth-child(2)");
// Здесь мы выбрали второй элемент списка ul, который содержит текст "Йончи".
// Можно также выбрать элемент по текстовому содержимому с помощью метода querySelectorAll и метода find:
const element2 = Array.from(document.querySelectorAll("li")).find(
  (li) => li.textContent === "Йончи"
);
console.log("element1:", element1, "element1:", element2);
// Оба варианта вернут элемент списка, содержащий текст "Йончи".
...
// Для получения коллекции элементов с классом "like" можно воспользоваться методом querySelectorAll:
const elements = document.querySelectorAll(".like");
console.log(elements);
// Этот метод вернет коллекцию всех элементов, у которых есть класс "like".
// В данном случае это первый и третий div.
...
/*
В документе, возможно, есть ещё «какой-то HTML-код»,
